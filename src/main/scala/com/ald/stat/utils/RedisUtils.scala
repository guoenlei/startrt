...
//  }
  ...
  def delKeys( keyList: List[String],redisPrefix: String):Unit = {
    val redisCache = CacheRedisFactory.getInstances(redisPrefix).asInstanceOf[AbstractRedisCache]
    use(redisCache.getResource) {
      resource => {
  ...
  /**
    ...
    * @param redisPrefix
    */
  def getClusterKeys(redisPrefix: String): ListBuffer[String] = {
    val conn = RedisCommandFactory.getInstances(redisPrefix)
    val pattern = s"$baseRedisKey:*"
    println(pattern)
    var pwd = "redis.write.pool.password"
  ...
  def migrate(): Unit = {
//    val oldRedisCache = CacheRedisFactory.getInstances("old").asInstanceOf[AbstractRedisCache]
//    val targetRedisCache = CacheRedisFactory.getInstances("default").asInstanceOf[AbstractRedisCache]
