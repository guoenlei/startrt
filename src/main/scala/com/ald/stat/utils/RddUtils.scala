...
object RddUtils {
  ...
  /**
    ...
    */
  //TODO:这里可能存在效率问题
  def checkAndSaveOffset(dateStr: String, topic: String, group: String, line: ConsumerRecord[String, String], resource: BaseRedisCache) = {
  ...
  /**
    ...
    * @return
    */
  def checkAndSaveMinOffset(dateStr: String, topic: String, group: String, line: ConsumerRecord[String, String], resource: BaseRedisCache) = {
  ...
  /**
    ...
    * @return
    */
  def checkAndSaveMinOffsetNew(dateStr: String, topic: String, group: String, minOffset: Long, partition: Int, resource: BaseRedisCache) = {
    val minOffsetKey = RedisUtils.buildOffsetKey(dateStr, topic, group, partition, "min") //当天最小的offset
    //维护当天最小的offset
    val minRedisOffset = resource.get(minOffsetKey)
    if (StringUtils.isNotBlank(minRedisOffset)) {
      if (minOffset < minRedisOffset.toLong) {
        resource.set(minOffsetKey, minOffset.toString)
      }
    } else {
      resource.set(minOffsetKey, minOffset.toString)
    }
  ...
  /**
    ...
    * @return
    */
  def checkAndSaveLatestOffset(dateStr: String, topic: String, group: String, line: ConsumerRecord[String, String], resource: BaseRedisCache) = {
    //每个分区进行和redis进行比较
    val latestOffsetKey = RedisUtils.buildOffsetKey(dateStr, topic, group, line.partition(), "latest")
  ...
  /**
    ...
    * @return
    */
  def checkAndSaveLatestOffsetNew(dateStr: String, topic: String, group: String, maxOffset: Long, partition: Int, resource: BaseRedisCache) = {
    //每个分区进行和redis进行比较
    val latestOffsetKey = RedisUtils.buildOffsetKey(dateStr, topic, group, partition, "latest")

    //最大offset处理
    val latestRedisOffset = resource.get(latestOffsetKey)
    if (StringUtils.isNotBlank(latestRedisOffset)) {
      //swap
      if (maxOffset > latestRedisOffset.toLong) {
        resource.set(latestOffsetKey, maxOffset.toString)
      }
    } else {
      resource.set(latestOffsetKey, maxOffset.toString)
    }
  ...
  /**
    ...
    * @param resource
    */
  def checkAndSaveOffset(dateStr: String, topic: String, group: String, offsetRanges: Array[OffsetRange], resource: BaseRedisCache) = {
  ...
  def shareRddUnionHandle[P <: DimensionKey : ClassTag : Ordering, K <: SessionSum](baseRedisKey: String, dateStr: String, shareRedisMark: String, backShareRedisMark: String, sessionRedisMark: String, share_pv_uv_rdd: RDD[(P, (Long, Long))], back_uv_op_rdd: RDD[(P, (Long, Long))], sessionRDD: RDD[(P, SessionSum)], redisPrefix: Broadcast[String]): RDD[(DimensionKey, ((Long, Long), ((Long, Long), SessionSum)))] = {
    ...
    share_pv_uv_rdd.fullOuterJoin((back_uv_op_rdd).fullOuterJoin(sessionRDD)).
      map(v => {
        ...
        val back_pv_uv = v._2._2.getOrElse(defaultTriple)._1.getOrElse(0l,0l)
