...
object TrendPatchAllTask extends TaskTrait{
  ...
  def dailyStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    val pvRDD = TrendDailyUVStat.statPatchSubPV(baseRedisKey, dateStr, DimensionKey, logRecordRdd, redisPrefix,patchHour,prefix)
    val uvRDD = TrendDailyUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyUidSubDimensionKey, logRecordRdd, redisPrefix)
  ...
  def hourlyStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    val pvRDD = TrendHourlyUVStat.statPatchSubPV(baseRedisKey, dateStr, HourDimensionKey, logRecordRdd, redisPrefix, patchHour, prefix)
    val uvRDD = TrendHourlyUVStat.statPatchUVNew(baseRedisKey, dateStr, HourUidSubDimensionKey, logRecordRdd, redisPrefix)
    // final rdd
    val finalRDD = pvRDD.join(uvRDD).join(sessionRDD)
