...
import com.ald.stat.appletJob.patch.patchTask.TrendPatchAllTask
...
object AnalysisTrendPatch extends AbstractBaseJob {
  ...
  def rddPatch(sparkConf: SparkConf, taskId: String, dateStr: String, partitionNumber: Int, patchType: String): Unit = {
    ...
    val re_rdd=stream_rdd.repartition(partitionNumber)

    handleNewUserForOldSDK(re_rdd, baseRedisKey, prefix)//标记新用户
