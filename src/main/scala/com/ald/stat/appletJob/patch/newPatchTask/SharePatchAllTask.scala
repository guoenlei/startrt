...
import com.ald.stat.component.dimension.share.pageShare.{DailyPageShareDimensionKey, DailyPageShareSessionSubDimensionKey, DailyPageShareUidSubDimensionKey}
import com.ald.stat.component.dimension.share.share._
import com.ald.stat.component.dimension.share.userShare.{DailyUserShareDimensionKey, DailyUserShareSessionSubDimensionKey, DailyUserShareUidSubDimensionKey}
...
object SharePatchAllTask extends TaskTrait{
  ...
  def shareDailyStat(baseRedisKey: String, taskId: String, dateStr: String, logRecord_click_rdd: RDD[LogRecord], logRecord_status_rdd: RDD[LogRecord],
    ...
    //分享人数  and 分享次数
//    val share_pv_uv_rdd = ShareDailyStatusUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, DailyShareUidSubDimensionKey, logRecord_status_rdd, redisPrefix)

    val share_pv = ShareDailyStatusUVStat.statPatchSubPV(baseRedisKey, dateStr, DailyShareDimensionKey, logRecord_status_rdd, redisPrefix,patchHour,prefix)

    val share_uv = ShareDailyStatusUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyShareUidSubDimensionKey, logRecord_status_rdd, redisPrefix)
    val share_pv_uv_rdd = share_pv.join(share_uv)

    val back_pv = ShareDailyClickUVStat.statPatchSubPV(baseRedisKey, dateStr, DailyShareDimensionKey, logRecord_click_rdd, redisPrefix,patchHour,prefix)

    val back_uv = ShareDailyClickUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyShareUidSubDimensionKey, logRecord_click_rdd, redisPrefix)

    val back_uv_op_rdd = back_pv.join(back_uv)
    //分享被打开次数(回流量)  and  分享被打开人数
//    val back_uv_op_rdd = ShareDailyClickUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, DailyShareUidSubDimensionKey, logRecord_click_rdd, redisPrefix)
...
//    //分享人数  and 分享次数
//    val share_pv_uv_rdd = ShareHourlyStatusUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, HourShareUidSubDimensionKey, logRecord_status_rdd, redisPrefix)
//    //分享被打开次数(回流量)  and  分享给打开人数
//    val back_uv_op_rdd = ShareHourlyClickUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, HourShareUidSubDimensionKey, logRecord_click_rdd, redisPrefix)

    val share_pv = ShareHourlyStatusUVStat.statPatchSubPV(baseRedisKey, dateStr, HourShareDimensionKey, logRecord_status_rdd, redisPrefix,patchHour,prefix)

    val share_uv = ShareHourlyStatusUVStat.statPatchUVNew(baseRedisKey, dateStr, HourShareUidSubDimensionKey, logRecord_status_rdd, redisPrefix)
    val share_pv_uv_rdd = share_pv.join(share_uv)

    val back_pv = ShareHourlyClickUVStat.statPatchSubPV(baseRedisKey, dateStr, HourShareDimensionKey, logRecord_click_rdd, redisPrefix,patchHour,prefix)

    val back_uv = ShareHourlyClickUVStat.statPatchUVNew(baseRedisKey, dateStr, HourShareUidSubDimensionKey, logRecord_click_rdd, redisPrefix)
...
//    //分享人数  and 分享次数
//    val share_pv_uv_rdd = PageShareDailyStatusUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, DailyPageShareUidSubDimensionKey, logRecord_status_rdd, redisPrefix)
//    //分享被打开次数(回流量)  and  分享给打开人数
//    val back_uv_op_rdd = PageShareDailyClickUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, DailyPageShareUidSubDimensionKey, logRecord_click_rdd, redisPrefix)

    val share_pv = PageShareDailyStatusUVStat.statPatchSubPV(baseRedisKey, dateStr, DailyPageShareDimensionKey, logRecord_status_rdd, redisPrefix,patchHour,prefix)

    val share_uv = PageShareDailyStatusUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyPageShareUidSubDimensionKey, logRecord_status_rdd, redisPrefix)
    val share_pv_uv_rdd = share_pv.join(share_uv)

    val back_pv = PageShareDailyClickUVStat.statPatchSubPV(baseRedisKey, dateStr, DailyPageShareDimensionKey, logRecord_click_rdd, redisPrefix,patchHour,prefix)

    val back_uv = PageShareDailyClickUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyPageShareUidSubDimensionKey, logRecord_click_rdd, redisPrefix)
...
//    //分享人数  and 分享次数
//    val share_pv_uv_rdd = UserShareDailyStatusUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, DailyUserShareUidSubDimensionKey, records_rdd_userShare_status, redisPrefix)
//    //分享被打开次数(回流量)  and  分享给打开人数
//    val back_uv_op_rdd = UserShareDailyClickUVStat.statIncreaseCacheWithPVNew(baseRedisKey, dateStr, DailyUserShareUidSubDimensionKey, records_rdd_userShare_click, redisPrefix)
//
    val share_pv = UserShareDailyStatusUVStat.statPatchSubPV(baseRedisKey, dateStr, DailyUserShareDimensionKey, records_rdd_userShare_status, redisPrefix,patchHour,prefix)

    val share_uv = UserShareDailyStatusUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyUserShareUidSubDimensionKey, records_rdd_userShare_status, redisPrefix)
    val share_pv_uv_rdd = share_pv.join(share_uv)

    val back_pv = UserShareDailyClickUVStat.statPatchSubPV(baseRedisKey, dateStr, DailyUserShareDimensionKey, records_rdd_userShare_click, redisPrefix,patchHour,prefix)

    val back_uv = UserShareDailyClickUVStat.statPatchUVNew(baseRedisKey, dateStr, DailyUserShareUidSubDimensionKey, records_rdd_userShare_click, redisPrefix)
