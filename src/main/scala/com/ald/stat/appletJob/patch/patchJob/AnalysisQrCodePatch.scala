...
object AnalysisQrCodePatch extends AbstractBaseJob {
  ...
  def rddPatch(sparkConf: SparkConf, taskId: String, dateStr: String, partitionNumber: Int, patchType: String): Unit = {
    ...
    val stream_rdd = KafkaConsume.rddFromOffsetRange(sparkContext, topic, group, baseRedisKey, offsetPrefix)
    //退出
    if (stream_rdd._2) {
      break()
    }
    handleNewUserForOldSDK(stream_rdd._1, baseRedisKey, prefix)

    val logRecordRdd: RDD[LogRecord] = stream_rdd._1.repartition(partitionNumber).mapPartitions(par => {
    ...
    QrCodePatchAllTask.qrDailyStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, grey_kafkaProducer, grap_map, isOnLine, redisPrefix)
    QrCodePatchAllTask.qrHourStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, grey_kafkaProducer, grap_map, isOnLine, redisPrefix)
    QrCodePatchAllTask.qrGroupDailyStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, grey_kafkaProducer, grap_map, isOnLine, redisPrefix)
    QrCodePatchAllTask.qrGroupHourStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, grey_kafkaProducer, grap_map, isOnLine, redisPrefix)
  }
}
