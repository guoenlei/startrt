package com.ald.stat.appletJob.debug.task
...
import com.ald.stat.component.dimension.trend.{DailySessionSubDimensionKey, DailyUidSubDimensionKey, HourSessionSubDimensionKey, HourUidSubDimensionKey}
...
object DebugTrendAllTask extends TaskTrait {
    ...
    finalRDD.foreachPartition(par => {
      par.foreach(row => {
        ...
        if (open_count != 0) {
          ...
          if (secondary_avg_stay_time == null)
            secondary_avg_stay_time = 0d
        }
        ...
        val sqlInsertOrUpdate =
          s"""
             |insert into aldstat_trend_analysis_debug
             |(
             |    app_key,
             |    day,
             |    new_comer_count,
             ...
             |avg_stay_time=ifnull(round(total_stay_time/total_page_count,2),0),
        ...
        //数据进入kafka
        sendToDebugKafka(kafka, sqlInsertOrUpdate) //如果上线状态为1，直接入生产kafka
  ...
  def hourlyStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    val sessionRDD = TrendHourlySessionStat.doCache(baseRedisKey, taskId, dateStr, tempSessMergeResult, HourSessionSubDimensionKey, redisPrefix)
    //uv pv
    val uvRDD = TrendHourlyUVStat.statIncreaseCacheWithPV(baseRedisKey, dateStr, HourUidSubDimensionKey, logRecordRdd, redisPrefix)
    // final rdd
    val finalRDD = uvRDD.join(sessionRDD)
    ...
    finalRDD.foreachPartition(par => {
      ...
      par.foreach(row => {
        ...
        val sqlInsertOrUpdate =
          s"""
             |insert into aldstat_hourly_trend_analysis_debug
             |(
             |    app_key,
             |    day,
             |    hour,
