...
import com.ald.stat.appletJob.debug.task.DebugTrendAllTask
...
object DebugTrend extends AbstractBaseJob {

  val prefix = "debug_trend"
  val offsetPrefix = "debug_trend_offset"
  val baseRedisKey = "rt";
...
//    val stream = KafkaConsume.getStream(ssc, group, topic) //用普通方式获取Dstram
    val stream = KafkaConsume.streamFromOffsetsDebug(ssc, group, topic, baseRedisKey, offsetPrefix)
    ...
    stream_rdd.foreachRDD(rdd => {
      ...
      //趋势分析
      DebugTrendAllTask.hourlyStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, grey_kafkaProducer, grap_map, isOnLine, redisPrefix)
      DebugTrendAllTask.dailyStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, grey_kafkaProducer, grap_map, isOnLine, redisPrefix)
    })
    ssc.start()
