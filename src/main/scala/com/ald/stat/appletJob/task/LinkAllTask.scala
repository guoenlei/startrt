...
import com.ald.stat.component.dimension.link.summary._
...
object LinkAllTask extends TaskTrait {
  ...
  def allStat_first(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    summaryHourStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer,grey_kafkaProducer, grey_map, isOnLine, redisPrefix)
  ...
  def summaryStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    //新增授权
    val authUserRDD = LinkSummaryAuthStat.newAuthUserWithUV(baseRedisKey, dateStr,LinkImgSubDimensionKey,logRecordRdd, redisPrefix)
    //最终结果集
  ...
  /**
    ...
    * @param kafkaProducer
    */
  def summaryHourStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
                  kafkaProducer: Broadcast[KafkaSink[String, String]],grey_kafkaProducer: Broadcast[KafkaSink[String, String]],
                  grey_map:util.HashMap[String, String],isOnLine:Boolean, redisPrefix: Broadcast[String]): Unit = {

    val tempSessMergeResult = LinkSummaryHourSessionStat.stat(logRecordRdd, LinkHourSessionSubDimensionKey, SessionBaseImpl)
    //session处理
    val sessionRDD = LinkSummaryHourSessionStat.doCache(baseRedisKey, taskId, dateStr, tempSessMergeResult, LinkHourSessionSubDimensionKey, redisPrefix)
    //uv
    val uvRDD = LinkSummaryHourUVStat.statIncreaseCacheWithPV(baseRedisKey, dateStr, LinkHourUidSubDimensionKey, logRecordRdd, redisPrefix)
    //新增授权
    val authUserRDD = LinkSummaryHourAuthStat.newAuthUserWithUV(baseRedisKey, dateStr,LinkHourImgSubDimensionKey,logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
  ...
  def dailyLinkStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    //新增授权
    val authUserRDD = LinkDailyAuthStat.newAuthUserWithUV(baseRedisKey, dateStr, DailyLinkImgSubDimensionKey, logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
  ...
  def hourLinkStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    val authUserRDD = LinkHourlyAuthStat.newAuthUserWithUV(baseRedisKey, dateStr, HourLinkImgSubDimensionKey, logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
  ...
  def dailyMediaStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    //新增授权
    val authUserRDD = MediaDailyAuthStat.newAuthUserWithUV(baseRedisKey, dateStr, DailyMediaImgSubDimensionKey, logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
  ...
  def hourMediaStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    //新增授权
    val authUserRDD = MediaHourlyAuthStat.newAuthUserWithUV(baseRedisKey, dateStr, HourMediaImgSubDimensionKey, logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
  ...
  def dailyPositionStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    val authUserRDD = PositionDailyAuthStat.newAuthUserWithUV(baseRedisKey, dateStr, DailyPositionImgSubDimensionKey, logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
  ...
  def hourPositionStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord],
    ...
    //新增授权
    val authUserRDD = PositionHourlyAuthStat.newAuthUserWithUV(baseRedisKey, dateStr, HourPositionImgSubDimensionKey, logRecordRdd, redisPrefix)
    //最终结果集
    val finalRDD = uvRDD.join(sessionRDD).join(authUserRDD)
