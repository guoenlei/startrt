...
trait TaskTrait {
  ...
  val kafka_split_topic = ConfigUtils.getProperty("kafka.mysql.split.sql.topic")
  val grey_kafka_topic = ConfigUtils.getProperty("grey_kafka.mysql.sql.topic")
  val debug_kafka_topic = ConfigUtils.getProperty("debug_kafka.mysql.sql.topic")
  ...
  /**
    ...
    * @param sql
    */
  def sendToKafkaWithAK(kafka: KafkaSink[String, String], sql: String,app_key:String): Unit = {
    if ("true" == ConfigUtils.getProperty("write.to.kafka")) {
      kafka.send(kafka_split_topic, app_key,sql)
    } else {
      println(sql)
    }
  }
  ...
  /**
    ...
    * @param sql
    */
  def sendToDebugKafka(kafka: KafkaSink[String, String], sql: String): Unit = {
    if ("true" == ConfigUtils.getProperty("write.to.kafka")) {
      kafka.send(debug_kafka_topic, sql)
    } else {
      println(sql)
    }
  }
