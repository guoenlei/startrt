    ...
    finalRDD.foreachPartition(par => {
      ...
      par.foreach(row => {
        ...
        if (scene_open_count != 0) {
          secondary_stay_time = sessionSum.sessionDurationSum / scene_open_count
        }
        val one_page_count = sessionSum.onePageofSession
        //跳出率
        var bounce_rate = 0f
        if (scene_open_count != 0) {
          bounce_rate = one_page_count.toFloat / scene_open_count.toFloat
        }
        ...
        val sqlInsertOrUpdate =
          s"""
             ...
             |scene_open_count="$scene_open_count",
