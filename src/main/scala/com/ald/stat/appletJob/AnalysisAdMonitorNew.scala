...
object AnalysisAdMonitorNew extends AbstractBaseJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 对rdd中的json进行 解析
    val transform_stream = stream.mapPartitions(par => {
    ...
    transform_stream.foreachRDD(rdd => {
      val logRecordRdd_1: _root_.org.apache.spark.rdd.RDD[_root_.com.ald.stat.log.LogRecord] = convertLinkRDD(dateLong, rdd)

      val logRecordRdd_2 = convertTotalRDD(dateLong, rdd)

      //对link和total分别标记新用户
      val logRecordRdd_link = handleNewUserForNewAdMonitorOnline(logRecordRdd_1, currentTime, newUserHbaseColumn, newUserTableName)
      println("***********************logRecordRdd_link : Count is " + logRecordRdd_1.count() + "*************************")

      val logRecordRdd_total = handleNewUserForNewAdMonitorOnline(logRecordRdd_2, currentTime, newUserHbaseColumn, newUserTableName)
      println("***********************logRecordRdd_total : Count is " + logRecordRdd_2.count() + "*************************")
